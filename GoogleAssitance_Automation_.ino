...
Adafruit_MQTT_Subscribe Light1 = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/Relay 1"); // FeedName
...
void loop() {
  ...
  while ((subscription = mqtt.readSubscription(20000))) {
    ...
    if (subscription == &Light3) {
      ...
      byte Light3_State = atoi((char *)Light3.lastread);
      digitalWrite(Relay3, Light3_State);
    }
    if (subscription == &Light4) {
      Serial.print(F("Got: "));
      Serial.println((char *)Light4.lastread);
      byte Light4_State = atoi((char *)Light4.lastread);
